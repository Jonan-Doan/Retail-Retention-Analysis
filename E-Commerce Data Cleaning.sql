...
WITH Product_Dim AS (
    SELECT [Item_Code], [Price], 'Electric household essentials' AS Category 
    FROM [Electric Household Essentials]
    UNION ALL 
    SELECT [Item_Code], [Price], 'Furniture' AS Category 
    FROM [Furniture]
), 

/* 
Used an optimized JOIN to fill in the blanks. 
If an item was missing its ID but had a price and category, I mapped it back to the correct product. 
I used a 'REC_' prefix for any category I didn't have specific IDs for yet, to keep my revenue totals 100% accurate. 
If quantity was missing, I used (Total Spent / Price) to find the missing number, defaulting to 1 where no other math was possible. 
*/
...
-- Final Output (Maintained at strict transactional granularity)
SELECT 
    [Transaction_ID],
    [Customer_ID],
    [Category],
